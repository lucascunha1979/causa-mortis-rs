...
export function buildZip(entries: ZipEntry[]): Uint8Array<ArrayBuffer> {
  ...
  for (const entry of entries) {
    ...
    const payload = entry.deflated ?? entry.data;
    const method = entry.deflated ? METHOD_DEFLATED : METHOD_STORED;
    ...
    localParts.push(localHeaderBytes, nameBytes, payload);
    ...
    offset += localHeaderBytes.length + nameBytes.length + payload.length;
  }
