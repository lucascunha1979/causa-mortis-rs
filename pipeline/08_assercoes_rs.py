...
for l in range(L):
    for s in range(S):
        for y in range(Y):
            o = D["overall"][l][s][y]; dd = np.array(D["deaths_by_age"][l][s][y], float); pp = np.array(D["population_by_age"][l][s][y], float)
...
for s in range(S):
    for y in range(Y):
        soma = sum(D["overall"][l][s][y][0] for l in range(1, L)); rs = D["overall"][0][s][y][0]
